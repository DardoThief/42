...
/*   Updated: 2019/08/09 14:15:43 by rrika            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	prep_a(t_stack *stack)
{
	...
	{
		c++;
		if (tmp->next && stack->b->num > tmp->num && stack->b->num \
				< tmp->next->num)
			break ;
		tmp = tmp->next;
	}
...
void	prep_b(t_stack *stack)
{
	...
	c = 0;
	tmp = stack->b;
	if (stack->a->num < ft_lstmin(stack->b) || stack->a->num > \
			ft_lstmax(stack->b))
		return (fix_b(stack));
	while (tmp)
	{
		c++;
		if (tmp->next && (stack->a->num < tmp->num && stack->a->num > \
					tmp->next->num))
			break ;
		tmp = tmp->next;
	}
